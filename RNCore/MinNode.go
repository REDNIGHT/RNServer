...
//
type MinNode2 struct {
	MinNode

	messageChan chan func(node IMessage)
}

func NewMinNode2(name string) MinNode2 {
	return MinNode2{MinNode{name}, make(chan func(IMessage))}
}

//IMessage
func (this *MinNode2) MessageChan() chan func(node IMessage) { return this.messageChan }
func (this *MinNode2) SendMessage(f func(IMessage)) {
	mc := this.MessageChan()
	mc <- f
	<-mc
}
func (this *MinNode2) OnMessage(f func(IMessage)) (close bool) {
	if f != nil {
		f(this)
